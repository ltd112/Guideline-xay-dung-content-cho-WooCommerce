# Hướng dẫn Sử dụng & Cấu hình Plugin EC Bulk Update

Tài liệu này hướng dẫn cách cấu hình và sử dụng plugin **EC Bulk Update** trên hệ thống WooCommerce. Plugin cung cấp giải pháp cập nhật hàng loạt (bulk update) sản phẩm và biến thể với hiệu năng cao, tích hợp bộ lọc động kiểu Shopify (Shopify-like dynamic filters), cơ chế chạy ngầm (background job processing), quản lý lịch sử và ghi nhận lỗi chi tiết.
...
Bộ lọc điều kiện của plugin được thiết kế trực quan tương tự Shopify/ShopBase, hỗ trợ thiết lập logic tổng hợp:
...
1.  Hệ thống sẽ chuyển hướng bạn về trang **Lịch sử (Bulk Update History)**.
